...
\begin{wrapfigure}{r}{0.4\textwidth}
  \centering
\includegraphics[scale=.3]{holonomics.png}
...
\subsubsection{Higher Strategy}

At a higher level our strategy system consists of a list of states, triggered in a specific order of priority. The states and their triggers can be seen in Table \ref{tab:machine}.To have a better understanding of the world we use the following tools in the to trigger the states:

\textbf{isSafeKick:}
First the intended ball trajectory vector is calculated. Second two more vectors are calculated between the source of the kick and each individual defending robot. Using a dot product the angle between these two vectors and the trajectory is calculated. The method returns True if this angle is less than a certain angle which is taken as a parameter to the method. 

\textbf{isCloserToGoal:}
This Method returns True if venus is in the best position to defend the goal and false if the friend is. We find the equation of the line to the center of the defending goal to the position of the enemy robot in possession of the ball. Then we find the perpendicular distance from both venus and our team mate to that line. If the robots do not lye perpendicular we use the distance to the attacking enemy instead. The method returns True if venus is in the best position to defend goal.

\textbf{hasBallInRange:}
...
\textbf{goalsideRobots:}
When trying to find the best position to move to receive a pass it was necessary to know about the enemy robot positions. To deduce whether they were not going to influence the pass a vector is constructed between both our team mate and venus. Two vectors perpendicular to this line are then constructed that pass through venus and our team mate respectively. These two vectors construct a channel and if an enemy robot lies within this channel it is not 'goalside'.   
...
\texttt{2} & \texttt{ATTACK\_PASS} &  \shortstack{Venus has ball and goal \textbf{isSafeKick} is False\\ and \textbf{isSafeKick} is True for passing to friend}\\ \hline
...
\texttt{5} & \texttt{ENEMY\_BALL\_TAKE\_PASS} &  Enemy has ball and friend \textbf{isCloserToGoal}\\ \hline
\texttt{6} & \texttt{FREE\_BALL\_YOURS} & \textbf{hasBallInRange} False for every robot\\ \hline
\texttt{7} & \texttt{RECEIVE\_PASS} &  Friend \textbf{hasBallInRange} and pass \textbf{isSafeKick} for friend\\ \hline
\texttt{8} & \texttt{FREE\_BALL\_NONE\_GOALSIDE} & No Enemy is in \textbf{goalsideRobots} \\ \hline
\texttt{8} & \texttt{FREE\_BALL\_1\_GOALSIDE} & Enemy 1 is in \textbf{goalsideRobots} \\ \hline
\texttt{8} & \texttt{FREE\_BALL\_2\_GOALSIDE} & Enemy 2 is in \textbf{goalsideRobots} \\ \hline
\texttt{8} & \texttt{FREE\_BALL\_BOTH\_GOALSIDE} &  Both Enemy are in \textbf{goalsideRobots}\\ \hline
