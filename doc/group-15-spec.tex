...
The updated system in Figure \ref{fig:venus1} used the value of a potential field obtain by summing the contributions of obstacles in the pitch each having a particular type of field. \begin{wrapfigure}{r}{0.55\textwidth}
  \begin{center}
...
\end{center}
	...
	\vspace{20pt}
	\label{fig:venus1}
\end{wrapfigure} The robot was modelled as an automaton in a finite grid with each square having its own potential. Using calibrated swerving motions from square to square the pitch could be navigated intelligently.

Navigational motions were added to a job list on the Arduino. This ensured the motion was continuous and not affected by lag. The high level planning was implemented by a state machine controlling on/off switches for each obstacles field and various addition fields imitating behaviours methods like intercept and grab ball. These addition fields worked for a universal spread of situations unlike the method built for milestone venus. The potential fields could be compounded together into states with ease, making high level planning easier to test and modular unlike the set of commands built in milestone venus.
\begin{wrapfigure}{r}{0.55\textwidth}
  \begin{center}
...
For the final system in Figure \ref{fig:venus2} the first large change was to the robot itself which was redesigned to be four wheeled and holonomoic. Simple motions like kicking and grabbing were implemented using the original wait till the first motion has stopped system right from the beginning. On top of this we added in a move forever method to the firmware which meant we could reach movements as granular as the speed of the communications system. Using this, a planner was built that fed the robot movement trajectories calculated straight from a vector field created using the potential fields in the previous design. This not only made the robot faster but it meant it could deal with more complicated mazes of potentials not capable by venus 1.0 as it could not change directions sharp enough. Also although the move forever function relied on the a lot on vision it still proved more precise than the job list in the previous design as the old robot would often move out of alignment with the grid that its motions were confined too. The final system choice was to relax undistortions of the vision feed to increase the reaction speed of the robot. This removed a lot of lag and meant the robot would only undertake useful tasks and in real time.
